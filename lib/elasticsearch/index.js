                ...
                appliedFilters.forEach(function (filter) {
                    if (checkIfObjectHasScope({ object: filter, scope: 'default' }) && Object.keys(filter.value).length) {
                        if (filter.attribute === 'custom') {
                            var value = filter.value, options = filter.options;
                            if (Object.keys(customFilters).includes(value)) {
                                queryChain = customFilters[filter.value]({ name: value, options: options, queryChain: queryChain, config: config });
                            }
                        }
                        else if (Object.keys(filter.value).every(function (v) { return (rangeOperators.indexOf(v) >= 0); })) {
                            ...
                            if (operator === 'or') {
                                if (filter.value === null) {
                                    queryChain = queryChain.orFilter('bool', function (b) {
                                        return b.notFilter('exists', mapping_1.default(config, filter.attribute));
                                    });
                                }
                                else {
                                    queryChain = queryChain.orFilter('terms', mapping_1.default(config, filter.attribute), filter.value);
                                }
                            }
                            else {
                                if (filter.value === null) {
                                    queryChain = queryChain.filter('exists', mapping_1.default(config, filter.attribute));
                                }
                                else {
                                    queryChain = queryChain.filter('terms', mapping_1.default(config, filter.attribute), filter.value);
                                }
                            }
                        }
                    }
